...
static void usage(const char *progname)
{
    fprintf(stderr,
    		...
    		// Marciel specific
            "Ping a CCN name prefix using Interests with name ccnx:/name/prefix/number.\n"
        ...
        enum ccn_upcall_kind kind, struct ccn_upcall_info* info)
{
    ...
    switch(kind) {
        ...
        case CCN_UPCALL_CONTENT:
            ...
            // Marciel specific get and display Content Data
         const unsigned char * data = NULL;
            size_t size;
            if (ccn_content_get_value(info->content_ccnb, info->pco->offset[CCN_PCO_E], info->pco, &data, &size) != 0)
            printf("Could not parse content value\n");
            else
            printf("%s\n", data);
            //printf("Content: %s.\n", data);
            // Marciel specific
