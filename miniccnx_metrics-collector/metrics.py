...
#class CCNAgent(object):
#    def __init__ (self, host):
#       host.cmd('ccnagent $NE_NAME &')  
...
class MetricsCollector(object):
	...
	self.indexMeasurement = {}
		...
		if ( newEntry['measurement'] in self.indexMeasurement ):
			 self.indexMeasurement[newEntry['measurement']] = self.indexMeasurement[newEntry['measurement']] + 1
		else:
			self.indexMeasurement[newEntry['measurement']] = 0

		allFields = dict(newEntry["fields"])

		for key, field in allFields.items(): 
			fileTransfer="/home/user/ccndStatus-ObjectValues/" + newEntry['tags']['hostname'] + '.' + newEntry['measurement'] + "." + key + '.' + str(self.indexMeasurement[newEntry['measurement']])

			self.sendToFile(fileTransfer, key , field )	
    
		
		fileTransfer="/home/user/ccndStatus-ObjectValues/" + newEntry['tags']['hostname'] + '.' + newEntry['measurement'] + ".hostname." + str(self.indexMeasurement[newEntry['measurement']])

		self.sendToFile(fileTransfer, 'hostname' , newEntry['tags']['hostname'] )

		fileTransfer="/home/user/ccndStatus-ObjectValues/" + newEntry['tags']['hostname'] + '.' + newEntry['measurement'] + ".time." + str(self.indexMeasurement[newEntry['measurement']])

		self.sendToFile(fileTransfer, 'time' , newEntry['time'])

                datapoints.append(newEntry)
        
	return datapoints

    def getValue( self, vector , index , field ):
	
	if ( vector[index] ):
		if ( 'fields' in vector[index] ):
			if ( field in vector[index]['fields'] ):
				return vector[index]['fields'][field]
		if ( 'tags' in vector[index] ):
			if ( field in vector[index]['tags'] ):
				return vector[index]['tags'][field]
		if ( 'time' in vector[index] ):
			if ( field in vector[index]['time'] ):
				return vector[index]['time'][field]
	if ( field in vector[index] ):
				return vector[index][field]
	
	return ""
	
    def sendToFile( self , filename , variable , value ):
	self.ccnhost.cmd('echo ' + variable + '=' + value + ' > ' + filename )
